...
// Cursor Rules Tokens
const CURSOR_RULES_VERSION = '0.0.1';
...
const WORKINDIA_CURSOR_RULES_FILE_PATH = join(
  CURSOR_RULES_TEMPLATE_DIRECTORY,
  'frontend-workindia-rules.mdc',
);

const CURSOR_RULES_FILE_NAME = `frontend-workindia-rules.mdc`;

const CONSUMER_CURSOR_RULES_RELATIVE_PATH = `.cursor/rules/${CURSOR_RULES_FILE_NAME}`;

const CHECK_CURSOR_RULES_DESCRIPTION = `Get the version from the workindia cursor rules file. If the file does not exist, send 0. 


Use this exact grep command: 
\`\`\`grep
grep -o "rules_version: [0-9.]*" .cursor/rules/frontend-workindia-rules.mdc
\`\`\`
`;

// Knowledgebase
const KNOWLEDGEBASE_DIRECTORY = join(PROJECT_ROOT_DIRECTORY, 'knowledgebase');
const COMPONENTS_KNOWLEDGEBASE_DIRECTORY = join(
  KNOWLEDGEBASE_DIRECTORY,
  'components',
);
const PATTERNS_KNOWLEDGEBASE_DIRECTORY = join(
  KNOWLEDGEBASE_DIRECTORY,
  'patterns',
);
const GENERAL_KNOWLEDGEBASE_DIRECTORY = join(
  KNOWLEDGEBASE_DIRECTORY,
  'general',
);
const ICONS_KNOWLEDGEBASE_DIRECTORY = join(KNOWLEDGEBASE_DIRECTORY, 'icons');
...
export {
  ...
  CURSOR_RULES_TEMPLATE_DIRECTORY,
  WORKINDIA_CURSOR_RULES_FILE_PATH,
