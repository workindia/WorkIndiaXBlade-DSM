...
```typescript
import { HeartStraightIcon } from '@workindia/dsm';
```
...
const Example = () => {
  return (
    <HeartStraightIcon size="medium" color="surface.icon.primary.normal" />
  );
};
```
...
HeartStraightFillIcon, HeartStraightOutlineIcon, HeartStraightIcon
