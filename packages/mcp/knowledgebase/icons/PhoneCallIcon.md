## Component Name

PhoneCallIcon

## Description

Represents phone calls, voice communication, or telephone functionality.

Standard icons provide a balanced appearance suitable for most use cases.

## Category

Call

## Variant

Standard

## When to Use

- Call-to-action buttons for phone
- Contact phone numbers
- Voice communication features

## When Not to Use

- For video calls (use VideoCameraIcon)
- For messaging (use EnvelopeIcon or ChatIcon)

## Import

```typescript
import { PhoneCallIcon } from '@workindia/dsm';
```

## Usage

```tsx
import { PhoneCallIcon } from '@workindia/dsm';

const Example = () => {
  return <PhoneCallIcon size="medium" color="surface.icon.primary.normal" />;
};
```

## Related Icons

PhoneCallFillIcon, PhoneCallOutlineIcon, PhoneCallIcon

## Design Guidelines

- Use icons consistently throughout your application
- Maintain appropriate size hierarchy (small: 16px, medium: 20px, large: 24px, xlarge: 32px)
- Use semantic colors from the design tokens
- Ensure sufficient contrast for accessibility
- Use filled variants for primary actions and outline variants for secondary actions
