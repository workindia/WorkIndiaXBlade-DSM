...
```typescript
import { TextAlignLeftIcon } from '@workindia/dsm';
```
...
const Example = () => {
  return (
    <TextAlignLeftIcon size="medium" color="surface.icon.primary.normal" />
  );
};
```
...
TextAlignLeftFillIcon, TextAlignLeftOutlineIcon, TextAlignLeftIcon
