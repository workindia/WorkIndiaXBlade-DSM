...
```typescript
import { BookmarkSimpleIcon } from '@workindia/dsm';
```
...
const Example = () => {
  return (
    <BookmarkSimpleIcon size="medium" color="surface.icon.primary.normal" />
  );
};
```
...
BookmarkSimpleFillIcon, BookmarkSimpleOutlineIcon, BookmarkSimpleIcon
