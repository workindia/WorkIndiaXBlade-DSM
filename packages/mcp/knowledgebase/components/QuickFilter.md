...
type QuickFilterContentProps = Pick<
  QuickFilterProps,
  'trailing' | 'value' | 'title'
> &
...
type QuickFilterGroupContextType = Pick<
...
```tsx
import { QuickFilterGroup, QuickFilter } from '@workindia/dsm';
import { Counter } from '@workindia/dsm';

function BasicQuickFilterExample() {
  const handleFilterChange = ({ name, values }) => {
    console.log(
      `Filter group ${name} has selected values: ${values.join(', ')}`,
    );
  };
