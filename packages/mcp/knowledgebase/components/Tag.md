...
import { Tag, FileTextIcon } from '@workindia/dsm';
...
```tsx
import React from 'react';
import { Tag, Box, FileTextIcon } from '@workindia/dsm';
    ...
    <Box display="flex" gap="spacing.4" alignItems="center">
      {mediumTagVisible ? (
        <Tag
          size="medium"
          icon={FileTextIcon}
          onDismiss={() => setMediumTagVisible(false)}
        >
          Medium Tag
        </Tag>
      ) : null}
      ...
      {largeTagVisible ? (
        <Tag
          size="large"
          icon={FileTextIcon}
          onDismiss={() => setLargeTagVisible(false)}
        >
          Large Tag
        </Tag>
      ) : null}
...
```tsx
import React from 'react';
import { Tag, Box, TextInput, Button, PlusIcon } from '@workindia/dsm';
...
function ComprehensiveTagExample() {
  ...
  const toggleTagDisabled = (tagId: string): void => {
    setTags(
      tags.map((tag) =>
        tag.id === tagId ? { ...tag, isDisabled: !tag.isDisabled } : tag,
      ),
    );
  };
