...
type SearchInputProps =
  | SearchInputPropsWithA11yLabel
  | SearchInputPropsWithLabel;
```
...
import { SearchInput, Box } from '@workindia/dsm';
...
function SearchWithDropdownExample() {
  ...
  const groupedItems: Record<string, typeof searchItems> = filteredItems.reduce(
    (acc, item) => {
      ...
      <Dropdown>
        ...
        <DropdownOverlay>
          {isLoading ? (
            ...
            <Box padding="spacing.4">No results found for "{searchTerm}"</Box>
          ) : (
            <ActionList>
