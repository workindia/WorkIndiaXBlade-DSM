...
type StepItemProps = {
   ...
   */
  titleColor?:
    | `feedback.text.${FeedbackColors}.intense`
      ...
      <StepGroup orientation="vertical" size="medium">
        {stepItems.map((stepInfo, index) => (
          <StepItem
            ...
            marker={
              <StepItemIndicator
                color={selectedIndex === index ? 'primary' : 'neutral'}
              />
            }
...
function NestedStepGroup() {
  ...
  return (
    ...
    >
      ...
      <Collapsible
        onExpandChange={({ isExpanded: newIsExpanded }) =>
...
```tsx
import { StepGroup, StepItem, StepItemIndicator, Box } from '@workindia/dsm';
