...
type TableProps<Item> = {
  ...
  sortFunctions?: Record<
    string,
    (array: TableNode<Item>[]) => TableNode<Item>[]
  >;
...
type TableBodyProps<Item> = {
  ...
  children:
    | React.ReactNode
    | ((tableItem: Item, index: number) => React.ReactElement);
};
...
const PaymentTable = () => {
  ...
  const payments: PaymentItem[] = Array.from({ length: 50 }, (_, i) => ({
    ...
    type: ['Payout', 'Refund'][Math.floor(Math.random() * 2)] as
      | 'Payout'
      | 'Refund',
  ...
  const sortFunctions = {
    ...
    AMOUNT: (array: TableNode<PaymentItem>[]) =>
                  ...
                  hoverActions={
                    <>
                      ...
                      <IconButton
                        ...
                        onClick={() =>
                          console.log('Approved', tableItem.paymentId)
                        }
                      />
                      <IconButton
                        icon={CloseIcon}
                        ...
                        onClick={() =>
                          console.log('Rejected', tableItem.paymentId)
                        }
                      />
                    </>
                  }
