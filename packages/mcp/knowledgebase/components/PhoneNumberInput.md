...
import { PhoneNumberInput, Box } from '@workindia/dsm';
      ...
      <PhoneNumberInput
        ...
        onCountryChange={({ country }) => {
          setPhoneData((prev) => ({
            ...
            country,
            isValid: prev.value
              ? isValidPhoneNumber(prev.value, country)
              : true,
          }));
        }}
...
```tsx
import { useState } from 'react';
import { PhoneNumberInput, Box, Text, PhoneIcon, Button } from '@workindia/dsm';
import type { CountryCodeType } from '@razorpay/i18nify-js';
...
```tsx
import { useState } from 'react';
import { PhoneNumberInput, Box, Text } from '@workindia/dsm';
...
```tsx
import { useState } from 'react';
import { PhoneNumberInput, TextInput, Box, Button } from '@workindia/dsm';
import { isValidPhoneNumber } from '@razorpay/i18nify-js';
import type { CountryCodeType } from '@razorpay/i18nify-js';
      ...
      <PhoneNumberInput
        ...
        onCountryChange={({ country }) => {
          ...
          if (
            ...
            isValidPhoneNumber(formData.phoneNumber, country)
          ) {
            setErrors((prev) => ({ ...prev, phoneNumber: '' }));
          } else if (formData.phoneNumber) {
            setErrors((prev) => ({
              ...prev,
              phoneNumber: 'Invalid phone number for selected country',
            }));
          }
        }}
