 ...
 */
type DatePickerProps<T extends DatePickerSelectionType = 'single'> =
  T extends 'single' ? SingleDatePickerProps : RangeDatePickerProps;

/**
 * Props for FilterChipDatePicker component
 */
type FilterChipDatePickerProps<T extends DatePickerSelectionType = 'single'> =
  Omit<
    DatePickerProps<T>,
...
import { DatePicker, Box, Text } from '@workindia/dsm';
...
```tsx
import React, { useState } from 'react';
import { DatePicker, Box, Text, Button } from '@workindia/dsm';
import dayjs from 'dayjs';
        ...
        <DatePicker
          ...
          footer={
            <Text size="small" color="surface.text.gray.muted">
              This section only displays records from the last 45 days. For
              earlier data, please download a report from the Reports section.
            </Text>
          }
        />
  ...
  return (
    <Box display="flex" flexDirection="column" gap="spacing.8">
      ...
      <Box>
        ...
        >
          Selected:{' '}
          {dateRange[0] ? dayjs(dateRange[0]).format('DD MMM YYYY') : 'None'} -
...
```tsx
import React, { useState } from 'react';
import { FilterChipDatePicker, Box, Text } from '@workindia/dsm';
import dayjs from 'dayjs';
  ...
  const datePresets = [
    {
      ...
      value: (date: Date): [Date, Date] => [
        dayjs(date).subtract(7, 'days').toDate(),
        date,
      ],
    },
    {
  ...
  return (
    <Box>
      ...
      <Box marginTop="spacing.3">
        <Text size="small">
          Single Date:{' '}
          {singleDate ? dayjs(singleDate).format('DD MMM YYYY') : 'None'}
        </Text>
        <Text size="small">
          Date Range:{' '}
          {dateRange[0] ? dayjs(dateRange[0]).format('DD MMM YYYY') : 'None'} -
