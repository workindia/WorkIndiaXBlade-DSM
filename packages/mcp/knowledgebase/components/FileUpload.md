...
import { FileUpload, TextInput, Button, Box } from '@workindia/dsm';

const SingleFileUploadExample = () => {
  const [name, setName] = useState('');
  const [fileList, setFileList] = useState([]);
  const [validationState, setValidationState] = useState<'none' | 'error'>(
    'none',
  );
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(
      `Form submitted: Name: ${name}, File: ${fileList[0]?.name || 'None'}`,
    );
  };
...
```tsx
import React, { useState } from 'react';
import { FileUpload, Box } from '@workindia/dsm';
import type { BladeFile, BladeFileList } from '@workindia/dsm';
  ...
  const simulateUpload = (file: BladeFile) => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress <= 100) {
        // Update progress
        setFileList((prevList) =>
          prevList.map((f) => (f.id === file.id ? { ...f, progress } : f)),
        );
      } else {
        // Complete the upload
